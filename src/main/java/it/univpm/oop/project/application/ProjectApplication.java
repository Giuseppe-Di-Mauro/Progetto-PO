...
import it.univpm.oop.project.model.CommentList;
...
public class ProjectApplication {
	...
	//private List<List<Comment>> list;
	...
	public CommandLineRunner run(RestTemplate restTemplate) throws Exception {
		return args -> {
			...
			for(int i=0; i<feed.getData().size() ; i++) {
				list.add(feed.getData().get(i).getComments().getData());
			}
			Repository repo = RepoFiller.fillRepo(list);
